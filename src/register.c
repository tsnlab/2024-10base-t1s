...
bool t1s_hw_readreg(struct ctrl_cmd_reg* p_regInfoInput, struct ctrl_cmd_reg* p_readRegData) {
    ...
    uint8_t txbuffer[MAX_REG_DATA_ONECONTROLCMD + HEADER_FOOTER_SIZE + REG_SIZE] = {0u};
    uint8_t rxbuffer[MAX_REG_DATA_ONECONTROLCMD + HEADER_FOOTER_SIZE + REG_SIZE] = {0u};
    uint16_t numberof_bytestosend = 0u;
    union ctrl_header_footer commandheader;
    union ctrl_header_footer commandheader_echoed;

    commandheader.ctrl_frame_head = 0u;
    commandheader_echoed.ctrl_frame_head = 0u;
    commandheader.ctrl_head_foot_bits.DNC = DNC_COMMANDTYPE_CONTROL;
    commandheader.ctrl_head_foot_bits.HDRB = 0u;
    commandheader.ctrl_head_foot_bits.WNR = REG_COMMAND_TYPE_READ; // Read from register
    if (p_regInfoInput->length != 0u) {
        commandheader.ctrl_head_foot_bits.AID =
            REG_ADDR_INCREMENT_ENABLE; // Read register continously from given address
    } else {
        commandheader.ctrl_head_foot_bits.AID = REG_ADDR_INCREMENT_DISABLE; // Read from same register
    }
    commandheader.ctrl_head_foot_bits.MMS = (uint32_t)p_regInfoInput->memorymap;
    commandheader.ctrl_head_foot_bits.ADDR = (uint32_t)p_regInfoInput->address;
    commandheader.ctrl_head_foot_bits.LEN = (uint32_t)(p_regInfoInput->length & 0x7Fu);
    commandheader.ctrl_head_foot_bits.P = ((get_parity(commandheader.ctrl_frame_head) == 0) ? 1 : 0);
    ...
    numberof_bytestosend =
        (uint16_t)(bufferindex + ((commandheader.ctrl_head_foot_bits.LEN + 1u) * REG_SIZE) +
    ...
    if (commandheader_echoed.ctrl_head_foot_bits.HDRB !=
        1u) // if MACPHY received header with parity error then it will be 1
    {
        if (commandheader.ctrl_head_foot_bits.LEN == 0u) {
            ...
            for (uint8_t regCount = 0u; regCount <= commandheader.ctrl_head_foot_bits.LEN; regCount++) {
                memmove((uint8_t*)&p_readRegData->databuffer[regCount],
                        &rxbuffer[ignored_echoedbytes + HEADER_FOOTER_SIZE + (REG_SIZE * regCount)],
                        REG_SIZE);
...
bool t1s_hw_writereg(struct ctrl_cmd_reg* p_regData) {
    ...
    commandheader.ctrl_head_foot_bits.DNC = DNC_COMMANDTYPE_CONTROL;
    commandheader.ctrl_head_foot_bits.HDRB = 0u;
    commandheader.ctrl_head_foot_bits.WNR = REG_COMMAND_TYPE_WRITE; // Write into register
    if (p_regData->length != 0u) {
        commandheader.ctrl_head_foot_bits.AID =
            REG_ADDR_INCREMENT_ENABLE; // Write register continously from given address
    } else {
        commandheader.ctrl_head_foot_bits.AID = REG_ADDR_INCREMENT_DISABLE; // Write into same register
    }
    commandheader.ctrl_head_foot_bits.MMS = (uint32_t)(p_regData->memorymap & 0x0Fu);
    commandheader.ctrl_head_foot_bits.ADDR = (uint32_t)p_regData->address;
    commandheader.ctrl_head_foot_bits.LEN = (uint32_t)(p_regData->length & 0x7Fu);
    commandheader.ctrl_head_foot_bits.P = ((get_parity(commandheader.ctrl_frame_head) == 0) ? 1 : 0);
    ...
    numberof_registerstosend = commandheader.ctrl_head_foot_bits.LEN + 1u;
    ...
    if (commandheader.ctrl_head_foot_bits.MMS == 0u) {
        struct ctrl_cmd_reg readreg_infoinput;
        struct ctrl_cmd_reg readreg_data;

        // Reads CONFIG0 register from MMS 0
        readreg_infoinput.memorymap = 0u;
        readreg_infoinput.length = 0u;
        readreg_infoinput.address = 0x0004u;
        memset(&readreg_infoinput.databuffer[0], 0u, MAX_REG_DATA_ONECHUNK);
        ...
        if (execution_status == false) {
            ...
            payload_sizeconfiguredval = (readreg_data.databuffer[0] & 0x00000007u);
    ...
    } else if (commandheader.ctrl_head_foot_bits.MMS == 1) {
        // CheckIfFCSEnabled();
    } else {
    }
