...
bool T1S_HW_ReadReg(stControlCmdReg_t* p_regInfoInput, stControlCmdReg_t* p_readRegData) {
    ...
    numberOfbytesToSend =
        (uint16_t)(bufferIndex + ((commandHeader.stVarHeadFootBits.LEN + 1) * EACH_REG_SIZE) +
                   igoredEchoedBytes); // Added extra 4 bytes because first 4 bytes during reception shall be ignored
        ...
        1) // if MACPHY received header with parity error then it will be 1
    {
        ...
        if (commandHeader.stVarHeadFootBits.LEN == 0) {
            memmove((uint8_t*)&p_readRegData->databuffer[0], &(rxBuffer[igoredEchoedBytes + HEADER_FOOTER_SIZE]),
                    EACH_REG_SIZE);
            ...
            for (uint8_t regCount = 0; regCount <= commandHeader.stVarHeadFootBits.LEN; regCount++) {
                memmove((uint8_t*)&p_readRegData->databuffer[regCount],
                        &rxBuffer[igoredEchoedBytes + HEADER_FOOTER_SIZE + (EACH_REG_SIZE * regCount)], EACH_REG_SIZE);
...
bool T1S_HW_WriteReg(stControlCmdReg_t* p_regData) {
    ...
    numberOfRegisterTosend =
        (uint8_t)(multiplicationFactor * ((uint8_t)commandHeader.stVarHeadFootBits.LEN + (uint8_t)1));
    ...
    for (uint8_t controlRegIndex = 0; controlRegIndex < numberOfRegisterTosend; controlRegIndex++) {
        for (int8_t regByteIndex = 3; regByteIndex >= 0; regByteIndex--) {
            txBuffer[bufferIndex++] = (uint8_t)((p_regData->databuffer[controlRegIndex] >> (8 * regByteIndex)) &
                                                0xFF); // Converting into big endian as MACPHY is big endian
        }
// ToDo if PROTE in CONFIG0 enabled then add 1's complement in between each register data(4bytes)
    ...
    // echoedAddOffset = (numberOfbytesToSend - igoredEchoedBytes) - 1; // -1 at last to get correct index value to
    // use(as bufferIndex was incremented at last)

    // ToDo Need to check CDPE bit in STATUS0 register is set, which indicates writing into register failed and update
    // writeStatus to false
