...
static int lan865x_set_nodeid(struct lan865x_priv *priv, u32 node_id)
{
	...
	regval = (LAN8650_NODE_MAX_COUNT << NODE_ID_BITS_WIDTH) |
...
static int lan865x_ethtool_get_ts_info(struct net_device *netdev,
...
static int lan865x_get_ts_config(struct net_device *netdev, struct ifreq *ifr)
{
	...
	return copy_to_user(ifr->ifr_data, hwts_config, sizeof(*hwts_config)) ?
		       -EFAULT :
		       0;
}
...
static int lan865x_set_ts_config(struct net_device *netdev, struct ifreq *ifr)
{
	...
	return copy_from_user(hwts_config, ifr->ifr_data,
			      sizeof(*hwts_config)) ?
		       -EFAULT :
		       0;
}
...
{
	return ((addr[bit / NUM_OF_BITS_IN_BYTE]) >>
		(bit % NUM_OF_BITS_IN_BYTE)) &
	       1;
}
...
static void lan865x_multicast_work_handler(struct work_struct *work)
{
	struct lan865x_priv *priv =
		container_of(work, struct lan865x_priv, multicast_work);
	u32 regval = 0;
	int ret;
				       ...
				       struct net_device *netdev)
{
	...
	if (skb_shinfo(skb)->tx_flags & SKBTX_HW_TSTAMP) {
		...
		if (hwts_config.tx_type != HWTSTAMP_TX_ON) {
			...
			priv->waiting_txts_skb[LAN865X_TIMESTAMP_ID_GPTP] =
				skb_get(cloned_skb);
			skb_shinfo(
				priv->waiting_txts_skb[LAN865X_TIMESTAMP_ID_GPTP])
			...
			priv->waiting_txts_skb[LAN865X_TIMESTAMP_ID_NORMAL] =
				skb_get(cloned_skb);
			skb_shinfo(priv->waiting_txts_skb
					   [LAN865X_TIMESTAMP_ID_NORMAL])
				->tx_flags |= SKBTX_IN_PROGRESS;
		}
	}
...
static int lan865x_netdev_ioctl(struct net_device *netdev, struct ifreq *ifr,
				int cmd)
{
	switch (cmd) {
	case SIOCGHWTSTAMP:
...
static long lan865x_ioctl(struct file *file, unsigned int cmd,
			  unsigned long arg);
...
static int lan865x_open(struct inode *inode, struct file *file)
{
	...
	struct spi_device *spi =
		container_of(file->private_data, struct spi_device, dev);
